...
public class BanCommand extends Command {
    ...
    public BanCommand(String name, String prefix, boolean enabled) {
        ...
        try {
            if(canBeExecuted(event)){
                ...
                if(event.getGuild().getUsers().contains(Base.logged.getUserByID(longIDF))){
                    ...
                    if(raison.equals(""))
                        event.getGuild().banUser(Base.logged.getUserByID(longIDF));
                    else
                        event.getGuild().banUser(Base.logged.getUserByID(longIDF), raison);
                    RequestBuffer.request(()->event.getChannel().sendMessage("L'utilisateur ID "+longIDF+" a été banni : "+(raison.equals("")?"Pas de raison":raison)));
                } else {
